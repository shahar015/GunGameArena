...
- `Progression.Promote()` increments `CurrentWeaponId` (public property with a private setter on the `MonoBehaviourSingleton<Progression>` instance) and calls `GameManager.Instance.EndGame()` when `CurrentWeaponId >= min(GameSettings.CurrentPool.GetWeaponCount(), WeaponCountOption.WeaponCount)`; otherwise `SpawnAndEquip()`.
- `WeaponCountOption` is a plain MonoBehaviour on the "Gun Game" board (static `int WeaponCount`; arrow buttons are `FVRPointableButton`s whose `OnPoint` is invoked directly by the hand via `Physics.Raycast` + `GetComponent<FVRPointable>()`, which returns the component even when it's disabled — so disabling the *pointable* only removes the hover highlight; making a button actually inert requires disabling its `Collider`(s) too, so the raycast never hits it).
...
- **Friendly fire**: Harmony prefix on `SosigLink.Damage(Damage)` returning `false` (skip) when Mode = Teams, `FriendlyFire` false, victim is a tracked team-0 sosig, and `d.Source_IFF == Roster.PlayerIff`. Patched here rather than `Sosig.ProcessDamage` because ProcessDamage only handles stagger/stun — `SosigLink.Damage` is what calls ProcessDamage and then applies health/limb damage via `DamageIntegrity` and bleeding, so blocking there is what actually stops the hit from hurting a teammate. `KillTracker.RecordHit` also early-returns on the same check, belt-and-braces, in case Harmony still calls its postfix.
- **Weapon-count lock**: `TeamMatch.ApplyWeaponCountLock()` finds `Object.FindObjectOfType<WeaponCountOption>()`; when Mode = Teams, disables every `Collider` on a button that targets it (this is what actually makes it inert — see "Verified GunGame facts" above), disables its `FVRPointableButton` too (kills the hover highlight), and sets alpha 0.4 on every `Text`/`Image` under it and under the counter text; otherwise restores (originals remembered). Re-locks if the found `WeaponCountOption` instance changes (e.g. after a scene change) instead of latching forever on the first one. Called after the Arena panel is built, on every panel Mode change, and on `RoundStarting`.
